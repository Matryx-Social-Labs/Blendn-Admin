...
/**
 * The one-time set-password link outlives the row it came from.
 *
 ...
 * unconfigured: toast, badge 7 → 6, no credential anywhere. The copy promises
...
describe("the approval credential is held by the list, not the row", () => {
  ...
  it("the row hands it up and keeps none of its own", () => {
    ...
    expect(row).toMatch(/onCredential\(\{ name: row\.display_name, email: result\.email, setPasswordLink: result\.setPasswordLink \}\)/)
    expect(row).not.toMatch(/useState<\{ email: string; setPasswordLink: string \} \| null>/)
