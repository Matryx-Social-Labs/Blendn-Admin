...
describe("retiring an amenity must not rewrite history", () => {
  ...
  it("carries is_active, so withdrawal is possible at all", () => {
    ...
    // Whitespace-insensitive: `prisma format` realigns a block whenever any
    // field in it changes, and the invariant here is the column and its
    // default, not the padding around them.
